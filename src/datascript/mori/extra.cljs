(ns datascript.mori.extra
  ...
  (:require-macros
   [datascript.mori.macros :refer [mori-export make-inspectable]])
...
(def mori-queue (fn [& args] (into cljs.core.PersistentQueue.EMPTY args)))
(mori-export queue mori-queue)
...
(mori-export toClj mori-toClj)
  
(defn- mori-configure [variable value]
    (case variable
      "print-length" (set! *print-length* value)
      "print-level" (set! *print-level* value)))
(mori-export configure mori-configure)
